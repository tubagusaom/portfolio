...
class Vm_sertifikat extends MY_Controller {

	function __construct() {
		parent::__construct();
		// has_privilege($this->uri->segment(1));
		$this->load->model('vm_sertifikat_model');
        $this->load->library('pagination');
	}

	function index(){
        $data = [
			'konten' => 'vm_sertifikat/index',
	...
	function datagrid(){
		...
		$data['record'] = $this->vm_sertifikat_model->data_jadwal();

		// var_dump($data); die();
        
        $view = $this->load->view('vm_sertifikat/grid',$data,TRUE);
        echo json_encode([
            'tabel' => $view
        ]);
    }
    ...
    function tambah(){
        ...
        $view = $this->load->view('vm_sertifikat/tambah',TRUE);
    ...
    function edit($id){
        ...
        $view = $this->load->view('vm_sertifikat/edit',$data,TRUE);
        echo json_encode($view);
    }
