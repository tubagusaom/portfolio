...
<form class="" action="" method="post" onsubmit="return checkform(this);">
	<table>
		...
		<tr>
			...
			?>
				...
				<a onclick="return confirm('Apakah Data Transaksi Jurnal akan di EXPORT ?')"
					href="module/jurnal/view/detail_jurnal.php?Export-JURNAL&&tglawal=<?php echo $_POST['tgl1'] ?>&&tglakhir=<?php echo $_POST['tgl2'] ?>">
					<!-- <input type="button" class="export-excel" name="export" value="Export Excel"> <i class="fa fa-home"></i> -->
					<button type="button" class="export-excel" name="export">Export <i
			...
			?>
				...
				<input type="date" name="tgl2" value="" class="acount"
					placeholder="Tanggal akhir ( Contoh:1945-08-17 )">
	...
	?>
		...
		<tr class="hover" bgcolor="<?php echo $warna ?>">
			...
			<td align="center">
				<a
					href="?Ubah-Jurnal&&header=<?php echo "Jurnal" ?>&&id=<?php echo "$data[0]" ?>&&inisial=<?php echo "$data[1]" ?>&&account=<?php echo "$data[8]" ?>&&saldo=<?php echo "$data[2]" ?>&&jenis=<?php echo "$data[3]" ?>&&reff=<?php echo "$data[4]" ?>&&ket=<?php echo "$data[5]" ?>&&date=<?php echo "$data[6]" ?>">Ubah</a>
