...
class Vm_skema extends MY_Controller {

	function __construct() {
		parent::__construct();
		// has_privilege($this->uri->segment(1));
		$this->load->model('vm_skema_model');
        $this->load->library('pagination');
	}

	function index(){
        $data = [
			'konten' => 'vm_skema/index',
	...
	function datagrid(){
		...
		$data['record'] = $this->vm_skema_model->data_jadwal();

		// var_dump($data); die();
        
        $view = $this->load->view('vm_skema/grid',$data,TRUE);
        echo json_encode([
            'tabel' => $view
        ]);
    }
    ...
    function tambah(){
        ...
        $view = $this->load->view('vm_skema/tambah',TRUE);
    ...
    function edit($id){
        ...
        $view = $this->load->view('vm_skema/edit',$data,TRUE);
        echo json_encode($view);
    }
