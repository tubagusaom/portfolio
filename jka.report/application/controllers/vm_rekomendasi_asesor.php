...
class Vm_rekomendasi_asesor extends MY_Controller {

	function __construct() {
		parent::__construct();
		// has_privilege($this->uri->segment(1));
		$this->load->model('vm_rekomendasi_asesor_model');
        $this->load->library('pagination');
	}

	function index(){
        $data = [
			'konten' => 'vm_rekomendasi_asesor/index',
	...
	function datagrid(){
		...
		$data['record'] = $this->vm_rekomendasi_asesor_model->data_jadwal();

		// var_dump($data); die();
        
        $view = $this->load->view('vm_rekomendasi_asesor/grid',$data,TRUE);
        echo json_encode([
            'tabel' => $view
        ]);
    }
    ...
    function tambah(){
        ...
        $view = $this->load->view('vm_rekomendasi_asesor/tambah',TRUE);
    ...
    function edit($id){
        ...
        $view = $this->load->view('vm_rekomendasi_asesor/edit',$data,TRUE);
        echo json_encode($view);
    }
